...
import { deactivateUser } from "@/app/actions/user-actions"
  ...
  const confirmDeactivation = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Reset form
        const form = document.getElementById("deactivation-form") as HTMLFormElement
        form?.reset()
      } else {
        setMessage({ type: "error", text: result.error || "Deactivation failed" })
      }
    } catch (error) {
  ...
  return (
    <Card className="bg-pink-950/20 border-pink-500/30">
      <CardContent className="p-6">
        {!showConfirmation ? (
          <form id="deactivation-form" action={handleSubmit} className="space-y-4">
            {/* Email Input */}
            ...
            <div className="flex items-start gap-3 p-4 bg-yellow-900/30 border border-yellow-500/50 rounded-md">
              ...
              <div className="text-sm text-yellow-200">
                ...
                <p>
                  Deactivating a user account will prevent them from accessing the system. This action can be reversed
          ...
          <div className="space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold text-white mb-2">Confirm Account Deactivation</h3>
              <p className="text-pink-200 mb-4">Are you sure you want to deactivate the account for:</p>
