...
export default function FollowingView({
  ...
  const getAllAvailableUsers = async () => {
    try {
      ...
      // Get unique majors
      const uniqueMajors = Array.from(new Set(result.map((user: User) => user.major)));
  ...
  const getFollowingList = async () => {
    try {
      const response = await fetch(`http://localhost:8888/get-user-follow/${userId}`, {
        method: "GET",
        headers: {
  ...
  const getPendingRequests = async () => {
    try {
      const response = await fetch("http://localhost:8888/get-all-requests-by-user", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const getPendingReceived = async () => {
    try {
      const response = await fetch("http://localhost:8888/get-all-requests-by-requestee", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const userDetails = await Promise.all(
        pendingRequests.map(async (req: any) => {
          const userRes = await fetch(`http://localhost:8888/get-user/${req.userId}`);
  ...
  const handleAcceptRequest = async (requestUserId: string) => {
    try {
      ...
      const deleteResponse = await fetch("http://localhost:8888/cancel-follow-request", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
                ...
                filteredDiscoverable.map((student) => (
                  ...
                  >
                    ...
                    <div className="flex space-x-2">
                      ...
                      >
                        {(() => {
                          if (followingList.some(f => f.id === student.id)) {
                            return "Following";
