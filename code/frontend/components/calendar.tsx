...
export default function Calendar() {
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(<div key={`empty-${i}`} className="h-14 md:h-24 p-1 border border-gray-800"></div>)
    }
      ...
      days.push(
        ...
        >
          <div className="flex justify-between">
            <span className={cn("font-medium text-sm", isToday && "text-pink-400")}>{day}</span>
          </div>
          <div className="mt-1 space-y-1">
            ...
            {dayEvents.length > 2 && <div className="text-xs text-pink-400">+{dayEvents.length - 2} more</div>}
          </div>
        </div>,
      )
    }
    ...
    <Card className="border-gray-800 bg-gray-950 text-gray-100 shadow-xl">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            <Button
              variant="outline"
              size="sm"
              className={cn("border-gray-700", viewMode === "following" ? "bg-gray-800" : "bg-transparent")}
              onClick={() => setViewMode(viewMode === "personal" ? "following" : "personal")}
            >
            ...
            <Popover open={showEventForm} onOpenChange={setShowEventForm}>
              ...
              <PopoverContent className="w-80 bg-gray-900 border-gray-700 text-gray-100">
                <EventForm selectedDate={selectedDate} onClose={() => setShowEventForm(false)} />
              </PopoverContent>
            </Popover>
          </div>
        </div>
      ...
      <CardContent className="p-4">
        <div className="grid grid-cols-7 gap-0">
          {DAYS.map((day) => (
            <div key={day} className="h-10 flex items-center justify-center font-semibold text-pink-300">
              {day}
            </div>
          ))}
          {renderCalendarDays()}
          ...
          <div className="space-y-3">
            {getSelectedDateEvents().length > 0 ? (
              getSelectedDateEvents().map((event, idx) => (
                <div key={idx} className="p-3 rounded-lg bg-gray-900 border border-gray-800">
                  <div className="flex justify-between items-start">
                    <h4 className="font-medium text-pink-300">{event.title}</h4>
                    <span className="text-sm bg-pink-900/50 px-2 py-0.5 rounded text-pink-200">{event.time}</span>
                  </div>
                  <div className="mt-1 text-sm text-gray-400">
                    Category:{" "}
                    <Badge variant="outline" className="ml-1 text-pink-400 capitalize">
                      {event.category}
                    </Badge>
                  </div>
                    ...
                    <div className="mt-3">
                      ...
                      <div className="flex -space-x-2">
                        {event.participants.map((participant, i) => (
                          <Avatar key={i} className="border-2 border-gray-900 h-6 w-6">
                        ...
                        <Button variant="outline" size="sm" className="h-6 text-xs ml-2 border-gray-700">
