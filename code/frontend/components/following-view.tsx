...
export default function FollowingView({ following }: FollowingViewProps) {
  ...
  const [discoverableStudents, setDiscoverableStudents] = useState(DISCOVERABLE_STUDENTS)
  ...
  const handleFollowToggle = (studentId: string, isDiscoverable = false) => {
    if (isDiscoverable) {
      ...
      if (student) {
        ...
        setFollowingList([...followingList, { ...student, mutualFollow: false }])
        // Remove from discoverable list
        setDiscoverableStudents(discoverableStudents.filter((s) => s.id !== studentId))
      }
    } else {
      // Toggle follow status in the following list
      setFollowingList(
        followingList.map((student) =>
          student.id === studentId ? { ...student, mutualFollow: !student.mutualFollow } : student,
        ),
      )
    }
  }
      ...
      <CardContent className="p-6">
        <Tabs defaultValue="following" className="w-full">
          <TabsList className="mb-6 bg-gray-900">
            <TabsTrigger value="following" className="data-[state=active]:bg-pink-900 data-[state=active]:text-white">
              <Users className="h-4 w-4 mr-2" />
              Following ({followingList.length})
            </TabsTrigger>
            <TabsTrigger value="discover" className="data-[state=active]:bg-pink-900 data-[state=active]:text-white">
                  ...
                  <div key={student.id} className="flex items-center justify-between p-4 bg-gray-800 rounded-lg">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <Button
                        size="sm"
                        className={
                          student.mutualFollow ? "bg-pink-800 hover:bg-pink-700" : "bg-gray-700 hover:bg-gray-600"
                        }
