...
import CommonFreeTime from "@/components/common-free-time"
import GroupWorkRecommendations from "@/components/group-work-recommendations"
...
export default function ScheduleView({
  ...
  const [selectedDayEvents, setSelectedDayEvents] = useState<(Schedule & { user?: User })[]>([])
  ...
  const getEventsForTimeSlot = (day: string, timeSlot: string) => {
    ...
    const dayDate = addDays(startOfWeek(currentDate, { weekStartsOn: 1 }), dayIndex)

    return schedules.flatMap(({ userId, user, events }) =>
      events
        .filter((event) => {
          const dayName = getDayNameFromIndex(dayIndex)
          const slotHour = Number.parseInt(timeSlot.split(":")[0], 10)
          ...
          if (eventEndHour === 0) {
            return event.day === dayName && eventStartHour <= slotHour
          }

          return event.day === dayName && eventStartHour <= slotHour && eventEndHour > slotHour
        })
  ...
  const getEventsForDay = (date: Date) => {
    const dayName = FULL_DAYS[getDay(date) === 0 ? 6 : getDay(date) - 1] // Adjust for Monday start
    const dateStr = format(date, "yyyy-MM-dd")

    return schedules.flatMap(({ userId, user, events }) =>
      events.filter((event) => event.day === dayName).map((event) => ({ ...event, user })),
    )
  }

  // Get events for the selected day (for day view)
  const getEventsForSelectedDay = () => {
    const dayName = FULL_DAYS[getDay(currentDate) === 0 ? 6 : getDay(currentDate) - 1] // Adjust for Monday start
  ...
  const renderMonthView = () => {
    ...
    const dayHeaders = DAYS_OF_WEEK.map((day) => (
      <div key={`header-${day}`} className="h-10 flex items-center justify-center font-semibold text-pink-300">
        {day}
      </div>
    ))
        ...
        days.push(
          ...
          >
            <div className="flex justify-between">
              <span className={`font-medium text-sm ${isToday(day) ? "text-pink-400" : ""}`}>{format(day, "d")}</span>
            </div>
            <div className="mt-1 space-y-1">
              ...
              {dayEvents.length > 3 && <div className="text-xs text-pink-400">+{dayEvents.length - 3} more</div>}
            </div>
          </div>,
        )
            ...
            <div className="sticky top-0 z-20 bg-gray-900 border-b border-gray-800 flex">
              ...
              <div className="flex-1 grid grid-cols-7">
                {DAYS_OF_WEEK.map((day, index) => {
                  const dayDate = addDays(startOfWeek(currentDate, { weekStartsOn: 1 }), index)
                  return (
                    <div
                      key={day}
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-400">{getDateForDay(index)}</div>
                    </div>
                  )
                })}
              </div>
                ...
                <div className="flex-1 grid grid-cols-7">
                  {DAYS_OF_WEEK.map((day, dayIndex) => (
                    <div key={day} className="col-span-1">
                      {TIME_SLOTS.map((time) => {
                        const events = getEventsForTimeSlot(day, time)
                        const dayDate = addDays(startOfWeek(currentDate, { weekStartsOn: 1 }), dayIndex)
                        const isCurrentTimeSlot =
                          isToday(dayDate) && new Date().getHours() === Number.parseInt(time.split(":")[0], 10)
                        ...
                        return (
                          ...
                          >
                            ...
                            {events.map((event, idx) => (
                              ...
                              >
                                <div className="h-1 w-full" style={{ backgroundColor: event.color }}></div>
                                <div className="p-1 text-xs">
                                  <div className="font-medium" style={{ color: event.color }}>
                                    {event.title}
                                  </div>
                                  <div className="text-[10px] text-gray-300">{event.course}</div>
                                </div>
                              </div>
                            ))}
                          </div>
                ...
                return (
                  <div
                    ...
                    className={`h-16 border-b border-gray-800 relative ${
                      isToday(currentDate) && new Date().getHours() === Number.parseInt(time) ? "bg-pink-900/10" : ""
                    }`}
                  >
                    {/* Current time indicator */}
                    {isToday(currentDate) && new Date().getHours() === Number.parseInt(time) && (
                      <div
                        className="absolute left-0 right-0 border-t-2 border-pink-500 z-10"
                        style={{ top: `${(new Date().getMinutes() / 60) * 100}%` }}
                      >
                    ...
                    {timeEvents.map((event, idx) => (
                      ...
                      >
                        ...
                        <div className="p-2">
                          ...
                          <div className="text-xs text-gray-300">{event.course}</div>
      ...
      <Card className="border-gray-800 bg-gray-950 text-gray-100 shadow-xl">
        <CardHeader className="bg-gray-900 border-b border-gray-800 p-4">
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-bold text-pink-400">Schedule Dashboard</h1>
            <Button
        ...
        <CardContent className="p-0 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 border-t border-gray-800">
            <ScheduleLegend schedules={schedules} />
            {selectedFriends.length > 0 && (
              <>
                <CommonFreeTime
                  userSchedule={schedules.find((s) => s.userId === "user-1")?.events || []}
