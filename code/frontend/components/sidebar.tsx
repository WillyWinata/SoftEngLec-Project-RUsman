    ...
    <div className="w-64 bg-gray-900 text-white min-h-screen p-4 border-r border-gray-800">
      ...
      <nav className="space-y-1 mb-6">
        ...
        <button
          ...
          className={`flex items-center space-x-3 p-3 rounded-lg transition-colors w-full text-left ${
            activeTab === "events" ? "bg-pink-900/50 text-pink-100" : "text-gray-300 hover:bg-gray-800 hover:text-white"
          }`}
        >
      ...
      <div className="mt-8">
        <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">View Other Schedules</h2>
            ...
            mutualFollows.map((friend) => (
              <div key={friend.id} className="flex items-center space-x-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor={`friend-${friend.id}`} className="text-sm text-gray-300 cursor-pointer">
                    {friend.name}
                    <div className="text-xs text-gray-500">{friend.department}</div>
                  </Label>
                </div>
              </div>
            ))
