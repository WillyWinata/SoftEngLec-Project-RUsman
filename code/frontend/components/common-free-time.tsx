...
export default function CommonFreeTime({ userSchedule, friendSchedules, selectedFriends }: CommonFreeTimeProps) {
  ...
  return (
    <div className="bg-gray-900 p-4 rounded-lg">
      <h3 className="text-lg font-semibold mb-3 text-pink-400">Common Free Time with {friendNames}</h3>
          ...
          <div key={day} className="space-y-2">
            ...
            {slots.length > 0 ? (
              <div className="space-y-1">
                {slots.map((slot, idx) => (
                  <div key={idx} className="text-xs p-1 bg-green-900/30 text-green-300 rounded">
                    {slot}
                  </div>
                ))}
              </div>
